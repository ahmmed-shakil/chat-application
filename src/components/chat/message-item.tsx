...
import { CheckCheck, Download, FileText, Image, Mic, Video } from 'lucide-react';
          ...
          <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
            ...
            <div className="flex-1">
              ...
              <audio controls className="w-full mt-1">
                <source src={`http://localhost:5000${message.content}`} type="audio/mpeg" />
                Your browser does not support the audio element.
              </audio>
            </div>
            <span className="text-xs text-gray-500">{getFormattedTime(message.createdAt)}</span>
          </div>
        );
      case 'video':
        return (
      ...
      <div className="flex items-end gap-2 max-w-[80%]">
        {showSender && (
          <Avatar className="w-6 h-6">
            <AvatarImage src={message.sender.profilePicture} alt={message.sender.name} />
            <AvatarFallback>
