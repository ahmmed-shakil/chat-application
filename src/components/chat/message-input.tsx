...
import { useSendMessageMutation, useUploadFileMutation } from '@/lib/features/message/messageApiSlice';
import { useSocket } from '@/contexts/SocketContext';
import { useToast } from '@/hooks/use-toast';
import { cn } from '@/lib/utils';

import {
  Paperclip, Send, Mic, Image, FileText, Video, X, Loader2
} from 'lucide-react';
      ...
      {selectedFile && (
        <div className="mb-2 p-2 bg-gray-100 dark:bg-gray-800 rounded-md flex items-center justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {fileType === 'video' && <Video className="h-5 w-5 text-blue-500" />}
            {fileType === 'audio' && <Mic className="h-5 w-5 text-blue-500" />}
            {fileType === 'file' && <FileText className="h-5 w-5 text-blue-500" />}
      ...
      <div className="flex items-end gap-2">
        ...
        <div className="relative flex-1">
          <Textarea
            ...
            className="resize-none py-3 pr-12 min-h-[2.5rem] max-h-32"
            value={message}
